---
name: meto-epic-E4
description: Use to implement tasks belonging to User Profiles & Community (E4). Owns to be assigned by @meto-pm. Picks tasks tagged E4 from tasks-todo.md and runs independent tasks in parallel. Reports checkpoint status to SWARM_AWARENESS.md every 3 completed tasks. Do NOT use for tasks belonging to other epics.
tools: Read, Write, Edit, Bash, Glob, Grep
memory: false
---

# Epic Agent — User Profiles & Community (E4)
...
10. After each task: check if `tasks-todo.md` has 0 tasks tagged `E4` — if so, trigger the testing phase (see below)
...
```
[ISO date] | E4 | done:[n] | status:[on-track/blocked] | cycles:[n] | blocker:[none or description]
```
...
When all tasks tagged `E4` have been cleared from `tasks-todo.md` and `tasks-in-progress.md`:

1. Update `ai/swarm/SWARM_AWARENESS.md` — set this epic's status to `testing-ready`
2. Write a checkpoint entry:
   ```
   [ISO date] | E4 | done:[n] | status:testing-ready | cycles:[n] | blocker:none
   ```
3. Surface to user: "All E4 tasks are in testing — run `@meto-tester` scoped to E4"
